...
public class MicronautBasePlugin implements Plugin<Project> {
    ...
    public void apply(Project project) {
        ...
        project.getPluginManager().apply(ConfigurationPropertiesPlugin.class);
        project.getPluginManager().apply(AggregatedJavadocParticipantPlugin.class);
        project.getPluginManager().apply(NativeImageSupportPlugin.class);
    }

    private void configureProjectVersion(Project project) {
        String version = project.getProviders().gradleProperty("projectVersion")
            .map(v -> {
