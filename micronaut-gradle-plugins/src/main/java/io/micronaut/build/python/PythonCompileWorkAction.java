...
public abstract class PythonCompileWorkAction implements WorkAction<PythonCompileParameters> {
     ...
     * Mirrors {@code PythonAnnotationProcessor.SOURCE_ROOT_OPTION}, which the build plugins cannot reference.
     */
    ...
    /**
     * Builds the compiler options: the source root option the plugin needs, followed by the
     * user-supplied arguments so that these are visible to the annotation processors as well.
     *
     * @param sourceRoot the absolute directory relative source directories are resolved against
